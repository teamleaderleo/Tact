...
`main`'s tip moves about **1.5 times an hour** (1,098 first-parent commits in 30 days, median gap 14.5 min, ~2.7/hour in the busiest evening hours). An earlier draft said 5/hour: that counted ~3,500 commits including the ones 176 non-squash merges brought in. Analysed by path, without building (a second session's work): ~56% of tip moves touch no macOS build input (web, `.github`, iOS, docs, tests, `cmux-tui/`), so a warmer can advance past them without a build once one replay confirms they really are near no-ops; ~32% touch package sources, `cmux.xcodeproj` or the package graph and are candidates for the slow 620 s kind (a prediction from paths, not a measurement). So a warmer has real work roughly every 40 minutes, not constantly. The pieces that make it work together:

1. **Merge queue**: `main` advances in batches, so there are fewer distinct states to warm and each warm slot stays valid longer.
