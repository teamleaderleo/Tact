...
- Type-check hotspot splits (#13105 carries the one with evidence), measured with `-warn-long-expression-type-checking=200`: `BrowserPanelView.swift` goes from 4 slow sites (worst one expression, ~7-8 s) to 0. `MarkdownPanelView` and `CustomSidebarPanelView` had **no** site over 200 ms before the change, so their splits have no evidence; my per-file ranking of them came from a contended cold build. No measurable whole-build change. Next slow site: `SurfaceCatalog+CloudPorts.swift:37` (2.7 s).
...
- Moving code out of the app module into packages: first extraction is #13107 (`CmuxComputerUse`, 10 files, ~2,900 lines). **Same-file comment edit: 38.9 s as an app-target file, 13.3 s once it lives in a package** (0 app-target files compiled); two more package-file samples 12.8 / 12.5 s; app-target comparator on the same branch 41.1 s. The first version did not build: `AgentPIDProcessIdentity.swift` was a member of both the app and the CLI target, and the CLI does not link the new package; it now lives in `CmuxFoundation`. Lessons for every extraction: check each moved file's target membership (`grep -c "<file> in Sources" project.pbxproj`), build the tests too (`build-for-testing`), and watch for `InternalImportsByDefault` in the destination package (`public import`). My own import script put imports inside `#if canImport` blocks in 25 test files; caught by the test build, not the app build.
...
- **Which extraction next? None of the easy ones.** The ten cleanest candidates from the dependency audit (at most 3 outside symbols each) together received 2.7% of the app module's file-touches on `main` in 30 days (98 of 3,576). 74% of touches land in files that hold a type from one dependency cycle of 972 types; `Sources/Cloud/` alone takes 26%, and it references `AppDelegate` at 59 sites and the surface catalog's model types at ~500. The per-edit win (40 s -> 13 s) only reaches code that can leave the module, and the code people edit cannot leave until that cycle is cut: catalog value types into a package with no app imports, then `AppDelegate` behind a protocol for Cloud. Design work for the owners; not attempted.
- **First cut of that cycle, built and measured (#13119, RFC #13108).** The surface catalog's value types and the snapshot parser (56 types, ~4,000 lines) moved into `CmuxSurfaceCatalogModel`. Same edits to `SurfaceCatalogModel.swift`, file in the app module vs in the package: comment 38.5 s -> 12.9 s; **interface change (new member on a widely used struct) 63.0 / 62.1 s -> 54.1 / 55.8 s**, 193 vs 199 app files recompiled. I had predicted interface edits would get slower in a package; they do not, because the in-module build already recompiles ~190 files for that edit. What the static dependency map missed and only a build showed: a type's `==` in a separate extension file, helpers embedded in two larger files, one file that was also a CLI-target member, and 27 structs needing explicit `public init`s. Also checked and dropped: `project.pbxproj`/`Localizable.xcstrings` as merge-conflict sources (1 and 0 conflicts in 100 open PRs test-merged), and moving app-hosted tests to packages (~8 of 984 files qualify).
