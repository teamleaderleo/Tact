...
| `CMUX_RELOAD_KEEP_RUNNING` | upstream [manaflow-ai/cmux#12962](https://github.com/manaflow-ai/cmux/pull/12962), **closed unmerged** 2026-09-19 by Leo; fork #60 closed too | The env-var opt-out leaves the running app and `cmuxd` on an old revision while files on disk are replaced. To be reworked as an explicit, visible build-only mode. Do not reopen or build on `contrib/reload-keep-running`; it also carries a maintainer commit (`dbb0ac0516`, Austin Wang) |
