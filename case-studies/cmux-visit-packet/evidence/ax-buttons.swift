// Dump every AXButton a running app exposes: its accessibility label, its
// identifier, its size and its supported actions.
//
// This is how the label defect in ../accessibility-labels.md was found, and how
// the fix was verified against two builds of the same cmux commit.
//
//   swiftc -O ax-buttons.swift -o ax-buttons
//   ./ax-buttons $(pgrep -f "cmux.app/Contents/MacOS/cmux" | head -1)
//
// The calling process must be trusted for Accessibility (System Settings >
// Privacy & Security > Accessibility). The program prints AXIsProcessTrusted to
// stderr so a false there explains an empty result.
//
// Read-only: it copies attributes and never performs an action.
