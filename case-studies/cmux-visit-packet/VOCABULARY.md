...
| **preserve object identity** | Causal handoff + Quick Look | A transition can change representation, tool, or amount of detail while the selected thing remains recognizably the same thing. |
| **selection receipt** | Causal handoff | A durable handle for the human act of pointing at a browser object, carrying exact and fallback context across terminal/agent/reload boundaries. |
| **causal ribbon** | Causal handoff | A compact index across linked selection → runtime/request → source → edit → build → verification events, with exact receipts one step away. |
| **active revision** | Causal handoff | The code/build generation the browser is actually running, kept distinct from working source that has merely changed on disk. |
| **claim proof** | Causal handoff | Replay and evidence attached to the exact user-facing behavior being verified. |
| **selection continuity** | Quick Look | Temporary inspection ends with the same object selected in the same list position, ready for the next action. |
| **inspect / keep** | Quick Look | Two verbs for two lifetimes: transient preview for a glance; durable pane surface for an artifact worth keeping beside the work. |
| **selection is object state; focus is input routing** | Pane selection/focus case | “Which surface is showing?” and “where will the next keystroke go?” deserve separate visual channels. |
| **three states, three channels** | Pane selection/focus case | Selected, focused, and hovered each get their own treatment instead of competing through one accent signal. |
