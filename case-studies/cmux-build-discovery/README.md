...
The observed candidate admission failure is tracked directly in Browser [#481](https://github.com/manaflow-ai/cmux-browser/issues/481).
...
These are proposed follow-ups, not completed capabilities. Browser [#479](https://github.com/manaflow-ai/cmux-browser/issues/479) owns recoverable metadata and contributor read access; Browser [#480](https://github.com/manaflow-ai/cmux-browser/issues/480) owns repeated host compilation. Tact links to those implementation owners. The metadata work must confirm authenticated index access and package-retention semantics. Do not add a competing scheduler or use persistent discovery as evidence that old binaries remain executable.

One access detail needs an explicit owner decision: the current [candidate transport](https://github.com/manaflow-ai/cmux-browser/blob/6696b66ec83925c616f869a2d4f89454046b4eb9/scripts/r2-candidate-transport.py) uses `R2_UPLOAD_TOKEN` for download as well as upload. GitHub metadata discovery alone does not grant package access. A contributor-facing download flow should use owner-approved read-scoped access or a bounded download grant, not require copying a CI uploader credential into every developer machine. The existing receipt's nonce-bearing object keys also mean a source SHA alone cannot reconstruct a lost receipt. Persist and resolve the original validated metadata; do not invent replacement object identities. Neither access policy nor infrastructure was changed in this slice.

A second concrete feedback-cost lead appeared in this PR's [CI run](https://github.com/manaflow-ai/cmux-browser/actions/runs/35526531693): changing the host-test registry triggers the updater workflow. Its Linux and macOS jobs each run five full host-suite iterations via [`stress-updater.sh`](https://github.com/manaflow-ai/cmux-browser/blob/6696b66ec83925c616f869a2d4f89454046b4eb9/scripts/stress-updater.sh), recompiling the C++ fixtures each iteration. Windows uses a different 25-iteration staging stress test; the platform jobs are not interchangeable. A focused follow-up should measure compile versus execution time and test compiling once per immutable invocation while preserving fresh-process repetitions, failure diagnostics, and every platform's coverage. Selective change routing is a separate experiment. No stress iterations or checks were removed by the discovery PR.
