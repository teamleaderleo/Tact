...
A second concrete feedback-cost lead appeared in this PR's [CI run](https://github.com/manaflow-ai/cmux-browser/actions/runs/35526531693): changing the host-test registry triggers the updater workflow. Its Linux and macOS jobs each invoke [`stress-updater.sh`](https://github.com/manaflow-ai/cmux-browser/blob/6696b66ec83925c616f869a2d4f89454046b4eb9/scripts/stress-updater.sh) five times through the full host suite, recompiling the C++ fixtures each iteration. Windows uses a different 25-iteration staging stress test; the platform jobs are not interchangeable. A focused follow-up should measure compile versus execution time and test compiling once per immutable invocation while preserving fresh-process repetitions, failure diagnostics, and every platform's coverage. Selective change routing is a separate experiment. No stress iterations or checks were removed by the discovery PR.
